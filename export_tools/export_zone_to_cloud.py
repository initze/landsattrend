...
parser.add_argument("--zone", help="The Individual zone")
...
if 'zone' in vars(args):
    if vars(args)['zone'] is not None:
        print("We have a zone")
        zone_value = vars(args)['zone']
        ZONE = zone_value
...
if __name__ == "__main__":
    ...
    region_values = regions[PROCESS_SITE]

    X_MIN_START = region_values['X_MIN_START']
    X_MIN_END = region_values['X_MIN_END']
    matching_lons = []
    bucket_name = "pdg-landsattrend"

    for x in range(X_MIN_START, X_MIN_END):
        zone = get_zone(x)
        if zone == ZONE:
            print(x, "This is in our zone")
            matching_lons.append(x)
        else:
            print(x, 'not in zone')

    new_region_values = {}
    new_region_values['X_MIN_START'] = min(matching_lons)
    new_region_values['X_MIN_END'] = max(matching_lons)
    new_region_values['Y_MIN_START'] = region_values['Y_MIN_START']
    new_region_values['Y_MIN_END'] = region_values['Y_MIN_END']


    files_to_upload = get_filenames_to_upload(new_region_values['Y_MIN_START'], new_region_values['Y_MIN_END'], new_region_values['X_MIN_START'], new_region_values['X_MIN_END'])

    try:
        run_export(Y_MIN_START=new_region_values['Y_MIN_START'], \
                   Y_MIN_END=new_region_values['Y_MIN_END'], \
                   X_MIN_START=new_region_values['X_MIN_START'], \
                   X_MIN_END=new_region_values['X_MIN_END'], \
